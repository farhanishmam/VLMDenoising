...
class VQADataset(Dataset):
    ...
    def getImageIdsAndNames(self):
        '''
        Returns the image IDs and names.
        ...
        for imageName in os.listdir(self.imageDirectory):
            ...
            match = re.search(r'(\d+)', imageName)  # Finds the numeric part in the filename
            if match:
                image_id = int(match.group(1))  # Convert the matched number to an integer
                imageIds.append(image_id)
                imageNames[image_id] = imageName
            else:
                print(f"Warning: Skipping file {imageName} (No numeric ID found)")
    ...
    #     Returns:
    #         imageId (int): Image ID extracted from the path.
    #     '''
    #     image_name = os.path.basename(image_path)  # Get the file name from the path
    #     image_id_str = image_name.split(".")[0].rpartition(self.imagePrefix)[-1]
    ...
    def get_image_id_from_path(self, image_path):
        ...
        # Use regex to extract the first numeric sequence in the filename
        match = re.search(r'(\d+)', image_name)
        if match:
            return int(match.group(1))  # Convert the matched number to an integer
        else:
            raise ValueError(f"Could not extract a numeric image ID from filename: {image_name}")
