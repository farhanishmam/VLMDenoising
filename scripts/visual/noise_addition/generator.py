...
class Generator():
    ...
    def __init__(self, dataset, logger):
        ...
        self.validTransformations = {
                                    ...
                                    #Fog           
                                    "Fog_L1": (self.transformToFog, 1),
                                    "Fog_L2": (self.transformToFog, 2),
                                    "Fog_L3": (self.transformToFog, 3),
                                    "Fog_L4": (self.transformToFog, 4),
                                    "Fog_L5": (self.transformToFog, 5),

                                    #Rain   
                                    "Rain_L1": (self.transformToRain, 1),
                                    "Rain_L2": (self.transformToRain, 2),
                                    "Rain_L3": (self.transformToRain, 3),
                                    "Rain_L4": (self.transformToRain, 4),
                                    "Rain_L5": (self.transformToRain, 5),

                                    #Frost 
                                    "Frost_L1": (self.transformToFrost, 1),
                                    "Frost_L2": (self.transformToFrost, 2),
                                    "Frost_L3": (self.transformToFrost, 3),
                                    "Frost_L4": (self.transformToFrost, 4),
                                    "Frost_L5": (self.transformToFrost, 5),
                                    
                                    #MotionBlur
                                    "Motion_Blur_L1": (self.transformToMotionBlur, 1),
                                    "Motion_Blur_L2": (self.transformToMotionBlur, 2),
                                    "Motion_Blur_L3": (self.transformToMotionBlur, 3),
                                    "Motion_Blur_L4": (self.transformToMotionBlur, 4),
                                    "Motion_Blur_L5": (self.transformToMotionBlur, 5),
                                    }
    ...
    def transformToMotionBlur(self, idx, severity=1):
        if(severity > 5):
            raise Exception("Severity must be <= 5")
        
        # Defining different kernel sizes for motion blur based on severity
        severity_dict = {
            1: 5,   # k=5 for severity 1
            2: 7,   # k=7 for severity 2
            3: 10,  # k=10 for severity 3
            4: 12,  # k=12 for severity 4
            5: 15   # k=15 for severity 5
        }
        
        k = severity_dict[severity]
        
        # Load image and normalize
        x, _, _, _, _, _ = self.dataset[idx]
        x = np.array(x) / 255.  # Normalize image to 0-1
        if x.shape[-1] == 4:
            x = x[:, :, :3]
        # Initialize motion blur augmenter
        aug = iaa.MotionBlur(k=k)

        # Apply motion blur to each color channel (RGB)
        augmented_image = aug(image=x)  # Apply motion blur to the entire image
        
        # Convert back to 0-255 range and RGB
        return cv2.cvtColor(np.float32(np.clip(augmented_image, 0, 1) * 255), cv2.COLOR_BGR2RGB)
            
    
    
    def transformToRain(self, idx, severity=1):
        ...
        rain_image = x.copy()
        num_drops = int(severity * 1000)

        for _ in range(num_drops):
            x1 = random.randint(0, x.shape[1])
            y1 = random.randint(0, x.shape[0])
            x2 = x1 + random.randint(-5, 5)
            y2 = y1 + random.randint(10, 20) * severity
            color = (random.randint(200, 255), random.randint(200, 255), random.randint(200, 255))
            cv2.line(rain_image, (x1, y1), (x2, y2), color, 1)

        blur_value = severity * 2 if severity > 1 else 3
        rain_image = cv2.blur(rain_image, (blur_value, blur_value))
        
        return cv2.cvtColor(rain_image, cv2.COLOR_BGR2RGB)
    
    

    def transformToFog(self, idx, severity=1):
        if severity < 1 or severity > 5:
            raise Exception("Severity must be between 1 and 5.")
        ...
        # Determine the level of fog intensity (the more severe, the more fog is added)
        fog_color = (200, 200, 200)  # Light gray for the fog
        num_fog_layers = int(severity * 5)  # Increase layers based on severity
        
        # Apply the fog effect with a semi-transparent overlay
        for _ in range(num_fog_layers):
            # Generate random position for the fog overlay
            x_offset = random.randint(0, x.shape[1] // 4)
            y_offset = random.randint(0, x.shape[0] // 4)
            fog_width = x.shape[1] - x_offset
            fog_height = x.shape[0] - y_offset
            
            # Create a semi-transparent fog layer
            fog_layer = np.full((fog_height, fog_width, 3), fog_color, dtype=np.uint8)

            # Randomize opacity based on severity (lower opacity for lower severity)
            alpha = 0.05 * severity  # Increase alpha for more opacity
            
            if(severity>=3):
                alpha = alpha/2
            
            # Use blending: mix the fog with the image based on alpha
            for c in range(3):  # RGB channels
                fog_image[y_offset:y_offset+fog_height, x_offset:x_offset+fog_width, c] = \
                    fog_image[y_offset:y_offset+fog_height, x_offset:x_offset+fog_width, c] * (1 - alpha) + fog_layer[:, :, c] * alpha

        # Apply a slight blur to the whole image to diffuse the fog and make it more natural
        blur_value = severity * 2 if severity > 1 else 3
        fog_image = cv2.blur(fog_image, (blur_value, blur_value))

        return cv2.cvtColor(fog_image, cv2.COLOR_BGR2RGB)

    
    
    def transformToFrost(self, idx, severity=1):
        if severity > 5:
            raise Exception("Severity must be <= 5")

        c = [(1, 0.4), (0.8, 0.6), (0.7, 0.7), (0.65, 0.7), (0.6, 0.75)][severity - 1]
        
        # Assuming your dataset returns the image at index idx as a PIL Image
        x, _, _, _, _, _ = self.dataset[idx]
        ...
        x = x.astype(np.uint8)  # Ensure the image is in uint8 format
        if x.shape[-1] == 4:
            x = x[:, :, :3]
        
        # Load the frost texture based on severity
        ...
        frost = frost[..., [2, 1, 0]]  # Convert from BGR to RGB
        frost_resized = cv2.resize(frost, (x.shape[1], x.shape[0]))  # Resize frost texture to match input image size

        # Apply frost effect
        new_img = np.clip(c[0] * np.array(x) + c[1] * frost_resized, 0, 255).astype(np.uint8)
        
        return cv2.cvtColor(new_img, cv2.COLOR_BGR2RGB)  # Convert to RGB for OpenCV
