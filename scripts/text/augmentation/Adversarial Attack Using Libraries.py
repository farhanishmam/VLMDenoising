...
original_df = pd.read_csv('[without images]1_correct_validation.csv')
...
# List to store the modified questions and corresponding function names
modified_data = []
...
def apply_perturbations(row):
    ...
    # Applying each function to the question
    for func in [substituteCharacterByOCR, substituteCharacterByKeyboard, insertCharacterRandomly, substituteCharacterRandomly, swapCharacterRandomly, delCharacterRandomly, uppercaseRandomly, repeatCharacterRandomly, leetLettersWithPerturbation, whiteSpacePerturbation, substitute_with_homoglyphs,synonym_replacement, swap_word, misspell_word, abbreviation_replacement, repeat_words, backTranslation, sentence_paraphrase]:
        try:
            ...
            print(f"Error applying {func.__name__} to question '{question}': {e}")

# Apply the perturbation functions to each question
original_df.apply(apply_perturbations, axis=1)

# Create the final DataFrame from the modified data
final_df = pd.DataFrame(modified_data)

# Save the final DataFrame to a CSV file
final_df.to_csv('FinalNoisyText.csv', index=False)

# Display the final DataFrame or save it to a CSV
print(final_df)
